...
void CheckLostResponseWithPiggyback(nlTestSuite * inSuite, void * inContext)
{
    ...
    NL_TEST_ASSERT(inSuite, receiverRc->IsAckPending());
    // Make sure receiver resends after sender does.
    receiverRc->SetConfig({
        2, // CHIP_CONFIG_MRP_DEFAULT_INITIAL_RETRY_INTERVAL
        2, // CHIP_CONFIG_MRP_DEFAULT_ACTIVE_RETRY_INTERVAL
    });
    ...
    // We resent our first message, which did make it to the app-level listener
    // on the receiver (because it's a duplicate) but did trigger a standalone
    // ack.
    NL_TEST_ASSERT(inSuite, gLoopback.mSentMessageCount == 4);
    NL_TEST_ASSERT(inSuite, gLoopback.mDroppedMessageCount == 1);
    NL_TEST_ASSERT(inSuite, !mockSender.IsOnMessageReceivedCalled);
    ...
    // 1 tick is 64 ms, sleep 65 ms to trigger re-transmit from receiver
    test_os_sleep_ms(65);
    ReliableMessageMgr::Timeout(&ctx.GetSystemLayer(), rm);
