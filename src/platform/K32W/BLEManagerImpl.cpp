...
CHIP_ERROR BLEManagerImpl::_Init()
{
    ...
    VerifyOrExit(Ble_ConfigureHostStackConfig() == TRUE,
                 err = CHIP_ERROR_INCORRECT_STATE);
    ...
    bleAppCreated = xTaskCreate(bleAppTask,
                        CHIP_DEVICE_CONFIG_BLE_APP_TASK_NAME,
                        CHIP_DEVICE_CONFIG_BLE_APP_TASK_STACK_SIZE / sizeof(StackType_t),
                        this,
                        CHIP_DEVICE_CONFIG_BLE_APP_TASK_PRIORITY,
                        NULL);
    VerifyOrExit(bleAppCreated == pdPASS, err = CHIP_ERROR_INCORRECT_STATE);

    /* BLE Radio Init */
    XCVR_TemperatureUpdate(BOARD_GetTemperature());
    VerifyOrExit(XCVR_Init(BLE_MODE, DR_2MBPS) == gXcvrSuccess_c,
                 err = CHIP_ERROR_INCORRECT_STATE);

    /* Create BLE Controller Task */
    VerifyOrExit(blekw_controller_init() == CHIP_NO_ERROR,
                 err = CHIP_ERROR_INCORRECT_STATE);

    /* Create BLE Host Task */
    VerifyOrExit(blekw_host_init() == CHIP_NO_ERROR,
                 err = CHIP_ERROR_INCORRECT_STATE);
    ...
    VerifyOrExit(GattServer_RegisterCallback(blekw_gatt_server_cb) == gBleSuccess_c,
                 err = CHIP_ERROR_INCORRECT_STATE);

    /* Wait until BLE Stack is ready */
    VerifyOrExit(OSA_EventWait(event_msg, CHIP_BLE_KW_EVNT_INIT_COMPLETE, TRUE,
                               CHIP_BLE_KW_EVNT_TIMEOUT, &flags) == osaStatus_Success,
                               err = CHIP_ERROR_INCORRECT_STATE);

    GattServer_RegisterHandlesForWriteNotifications(1, attChipRxHandle);

    SetFlag(mFlags, kFlag_K32WBLEStackInitialized, true);
    SetFlag(mFlags, kFlag_AdvertisingEnabled,
            CHIP_DEVICE_CONFIG_CHIPOBLE_ENABLE_ADVERTISING_AUTOSTART ? true : false);
...
bool BLEManagerImpl::SendWriteRequest(BLE_CONNECTION_OBJECT conId, const ChipBleUUID * svcId,
...
bool BLEManagerImpl::SendReadRequest(BLE_CONNECTION_OBJECT conId, const ChipBleUUID * svcId,
                                     const ChipBleUUID * charId, PacketBufferHandle pBuf)
{
...
BLEManagerImpl::ble_err_t BLEManagerImpl::blekw_send_event(int8_t connection_handle,
                                          uint16_t handle, uint8_t* data, uint32_t len)
{
    ...
    /************* Send the indication *************/
    if (OSA_EventClear(event_msg, CHIP_BLE_KW_EVNT_INDICATION_CONFIRMED |
                      CHIP_BLE_KW_EVNT_INDICATION_FAILED) != osaStatus_Success)
    {
        ChipLogProgress(DeviceLayer, "BLE Event - Can't clear OSA Events");
        return BLE_E_FAIL;
    }

    if (GattServer_SendInstantValueIndication(connection_handle, handle, len, data)
           != gBleSuccess_c)
    {
    ...
    if(OSA_EventWait(event_msg, CHIP_BLE_KW_EVNT_INDICATION_CONFIRMED |
                     CHIP_BLE_KW_EVNT_INDICATION_FAILED,
                     FALSE, CHIP_BLE_KW_EVNT_TIMEOUT, &event_mask) != osaStatus_Success)
    {
...
CHIP_ERROR BLEManagerImpl::blekw_controller_init(void)
{
    ...
    /* Task creation */
    if (pdPASS  != xTaskCreate(Controller_TaskHandler,
                               "controllerTask",
...
CHIP_ERROR BLEManagerImpl::blekw_host_init(void)
{
    ...
    /* Task creation */
    if (pdPASS != xTaskCreate(Host_Task,
                              "hostTask",
                              HOST_TASK_STACK_SIZE,
                              (void *) 0,
                              HOST_TASK_PRIORITY,
                              NULL))
    {
		                 ...
		                 gapAdvertisingData_t* adv, gapScanResponseData_t* scnrsp)
{
    ...
    OSA_EventClear(event_msg, (CHIP_BLE_KW_EVNT_ADV_SETUP_FAILED |
                   CHIP_BLE_KW_EVNT_ADV_PAR_SETUP_COMPLETE));
    ...
    if (OSA_EventWait(event_msg,
                     (CHIP_BLE_KW_EVNT_ADV_SETUP_FAILED |
                      CHIP_BLE_KW_EVNT_ADV_PAR_SETUP_COMPLETE),
                      FALSE, CHIP_BLE_KW_EVNT_TIMEOUT, &event_mask) !=
                      osaStatus_Success)
    {
    ...
    if (OSA_EventWait(event_msg, (CHIP_BLE_KW_EVNT_ADV_CHANGED | CHIP_BLE_KW_EVNT_ADV_FAILED),
                      FALSE, CHIP_BLE_KW_EVNT_TIMEOUT, &event_mask) != osaStatus_Success)
    {
    ...
    VerifyOrExit(GetFlag(mFlags, kFlag_K32WBLEStackInitialized), /* */);
        ...
        if (ConfigurationMgr().IsFullyProvisioned())
        {
            ...
            ChipLogProgress(DeviceLayer,
                            "CHIPoBLE advertising disabled because device is fully provisioned");
        }
...
exit:
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer,
                     "Disabling CHIPoBLE service due to error: %s", ErrorStr(err));
...
void BLEManagerImpl::bleAppTask(void * p_arg)
{
    while (1)
    {
        xEventGroupWaitBits(bleAppTaskLoopEvent,
                            LOOP_EV_BLE,
                            true,
                            false,
                            portMAX_DELAY);
         ...
         if(MSG_Pending(&blekw_msg_list))
         {
             ...
             else if(msg->type == BLE_KW_MSG_ATT_READ)
             {
                 ...
                 ChipLogProgress(DeviceLayer, "Attribute read request(d:%d, h:%d) .",
                                 att_rd_data->device_id, att_rd_data->handle);
                 /* TODO: */
             }
...
void BLEManagerImpl::HandleWriteEvent(blekw_msg_t* msg)
{
    ...
    {
        bleResult_t res = GattServer_SendAttributeWrittenStatus(att_wr_data->device_id,
        ...
        case gInternalError_c:
            /* Notify the CHIP that the BLE hardware report fail */
               ChipLogProgress(DeviceLayer,
                        ...
                        pGenericEvent->eventData.internalError.errorCode,
                        pGenericEvent->eventData.internalError.errorSource,
...
void BLEManagerImpl::blekw_gap_advertising_cb(gapAdvertisingEvent_t* pAdvertisingEvent)
{
    ...
    else
    {
        ...
        ChipLogProgress(DeviceLayer, "Advertising failed: event=%d reason=0x%04X\n",
                    pAdvertisingEvent->eventType, pAdvertisingEvent->eventData.failReason);
...
void BLEManagerImpl::blekw_gap_connection_cb(deviceId_t deviceId,
...
*******************************************************************************/
CHIP_ERROR BLEManagerImpl::blekw_msg_add_att_written(blekw_msg_type_t type,
		uint8_t device_id, uint16_t handle, uint8_t* data, uint16_t length)
{
    ...
    msg = (blekw_msg_t *) MSG_Alloc(sizeof (blekw_msg_t) +
                                   sizeof(blekw_att_written_data_t) +
                                   length);
...
CHIP_ERROR BLEManagerImpl::blekw_msg_add_att_read(blekw_msg_type_t type,
