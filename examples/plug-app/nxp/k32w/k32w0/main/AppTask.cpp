...
CHIP_ERROR AppTask::Init()
{
    ...
    sLightLED.Set(!PlugMgr().IsTurnedOff());
    UpdateClusterState();
...
void AppTask::LightActionEventHandler(AppEvent * aEvent)
{
    ...
    {
        action = static_cast<PlugManager::Action_t>(aEvent->LightEvent.Action);
    ...
    else if (aEvent->Type == AppEvent::kEventType_Button)
    {
        if (PlugMgr().IsTurnedOff())
        {
            action = PlugManager::TURNON_ACTION;
        }
        else
        {
            action = PlugManager::TURNOFF_ACTION;
        }
    }
    else
    {
        err    = APP_ERROR_UNHANDLED_EVENT;
        action = PlugManager::INVALID_ACTION;
    }
...
void AppTask::ActionInitiated(PlugManager::Action_t aAction, int32_t aActor)
{
    // start flashing the LEDs rapidly to indicate action initiation.
    if (aAction == PlugManager::TURNON_ACTION)
    {
        K32W_LOG("Turn on Action has been initiated")
    }
    else if (aAction == PlugManager::TURNOFF_ACTION)
    {
...
void AppTask::ActionCompleted(PlugManager::Action_t aAction)
{
...
void AppTask::PostTurnOnActionRequest(int32_t aActor, PlugManager::Action_t aAction)
{
    AppEvent event;
...
{
    uint8_t newValue = !PlugMgr().IsTurnedOff();
